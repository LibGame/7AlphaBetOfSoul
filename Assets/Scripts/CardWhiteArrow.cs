...
public class CardWhiteArrow : CardProperties
{
    private void Start()
    {
    ...
    protected override void OnMouseDown()
    {
        if (GameManager.IsCanTake && !_isLay)
        {
            ...
            GameManager.WhiteCardList[IDCard] = card; // заменяю карту в колоде на нову созданную карту
            card.GetComponent<CardProperties>().IDCard = IDCard; // передаю айди этой карты в новую созданную
            card.GetComponent<CardProperties>().BackGroundCard = BackGroundCard;
            _spriteRenderer.sortingOrder = 2; // Делаю карту на слой выше карты в колоде
            card.transform.SetParent(GameManager.WhiteArrowsParent.transform);
            base.OnMouseDown();
        }
